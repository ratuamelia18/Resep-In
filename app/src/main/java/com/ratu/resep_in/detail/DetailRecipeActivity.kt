...
import com.ratu.resep_in.profile.OtherProfileActivity
...
class DetailRecipeActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val recipeFromIntent = intent.getSerializableExtra("RECIPE_DATA") as? Recipe
        val recipeId = intent.getStringExtra("RECIPE_ID")

        if (recipeFromIntent != null) {
            currentRecipe = recipeFromIntent
            continueSetup()
    ...
    private fun fetchRecipeById(id: String) {
        FirebaseFirestore.getInstance().collection("resep").document(id)
            .get()
            .addOnSuccessListener { doc ->
                currentRecipe = doc.toObject(Recipe::class.java)?.apply { this.id = doc.id }
